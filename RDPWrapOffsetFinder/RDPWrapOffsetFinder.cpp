...
int main(int argc, char **argv)
{
    ...
    if (GetLastError() == ERROR_ENVVAR_NOT_FOUND)
        symPath = L"cache*;srv*https://msdl.microsoft.com/download/symbols";
    if (!SymInitializeW(hProcess, symPath, FALSE))
        return -1;
    if (argc >= 2)
        lstrcpyA(szTermsrv, argv[1]);
    else
        lstrcpyA(szTermsrv + GetSystemDirectoryA(szTermsrv, sizeof(szTermsrv) / sizeof(char)), "\\termsrv.dll");
#ifndef _WIN64
    PVOID OldValue;
    ...
    Wow64DisableWow64FsRedirection(&OldValue);
#endif // _WIN64
    if (!SymSrvGetFileIndexInfo(szTermsrv, &Info, 0))
        return -6;
#ifndef _WIN64
    Wow64RevertWow64FsRedirection(OldValue);
#endif // _WIN64
    ...
    if (!SymLoadModuleEx(hProcess, NULL, szTermsrv, NULL, ImageBase, SizeOfImage, NULL, 0))
        return -3;
    ...
    if (SymFromNameW(hProcess, L"CDefPolicy::Query", &symbol))
        ...
        puts("ERROR: CDefPolicy_Query not found");

    if (hResData->Value.dwFileVersionMS <= 0x00060001)
        return 0;
    ...
    if (SymFromNameW(hProcess, L"CSLQuery::Initialize", &symbol))
    {
        ...
        printf("\n[%hu.%hu.%hu.%hu-SLInit]\n",
               HIWORD(hResData->Value.dwFileVersionMS),
               LOWORD(hResData->Value.dwFileVersionMS),
               HIWORD(hResData->Value.dwFileVersionLS),
               LOWORD(hResData->Value.dwFileVersionLS));

        const wchar_t *symbolsToCheck[] = {
            L"CSLQuery::bServerSku",
            L"CSLQuery::bRemoteConnAllowed",
            L"CSLQuery::bFUSEnabled",
            L"CSLQuery::bAppServerAllowed",
            L"CSLQuery::bMultimonAllowed",
            L"CSLQuery::lMaxUserSessions",
            L"CSLQuery::ulMaxDebugSessions",
            L"CSLQuery::bInitialized"};

        for (const auto &symbolName : symbolsToCheck)
        {
            PrintSymbolOffset(hProcess, symbolName, arch);
        }
    }
    else
    {
